package Mojolicious::Plugin::TusuEPRenderer;
use Mojo::Base 'Tusu';

use Mojo::Template;
use Mojo::Util qw/encode md5_sum/;

__PACKAGE__->attr('handler' => 'tusu_ep');
...
sub register {
  ...
  $app->plugin('TusuEPLRenderer', $conf);
  
  # Config
  my $name     = $conf->{name}     || 'tusu_ep';
  ...
  $app->renderer->add_handler(
    $name => sub {
      ...
      # Render with epl
      return $r->handlers->{tusu_epl}->($r, $c, $output, $options);
    }
  );
}
